...
def filter_fdi_values(path):
    print("Filtering FDI values...")
    ...
    for i in range(len(red_files)):
        ...
        outfile_name = red_files[i].split('_L')[0] + '_FDI_Filtered.tif'
        print('***, outfile name: ', outfile_name)

        # Matriz booleana
        fdi_mask = (fdi2 >= -0.0022) & (fdi2 <= 0.0805)

        # Convertir matriz a mascara
        fdi_treshold = fdi_mask.astype(int)
        ...
        fdi_data.GetRasterBand(1).WriteArray(fdi_treshold)
